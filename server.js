...
async function getBranchPrefix(branchId, fallback = "SKU") {
  if (!branchId) return fallback;
  try {
    const row = await queryOne("SELECT name FROM branches WHERE id = ? LIMIT 1", [branchId]);
    if (!row?.name) return fallback;
    const name = row.name.trim();
    const words = name.split(/\s+/).filter((w) => !["in", "&", "and", "the", "of", "at"].includes(w.toLowerCase()));
    if (words.length >= 3) {
      return (words[0][0] + words[1][0] + words[2][0]).toUpperCase();
    }
    if (words.length === 2) {
      const w0 = words[0].replace(/[^a-zA-Z0-9]/g, "").toUpperCase();
      const w1 = words[1].replace(/[^a-zA-Z0-9]/g, "").toUpperCase();
      return (w0.slice(0, 2) + w1.slice(0, 1)).slice(0, 3);
    }
  ...
  try {
    ...
    try {
      ...
      await conn.query(`
        UPDATE devices d
        JOIN (
          SELECT business_id, branch_id, MIN(id) as default_user_id 
          FROM users 
          WHERE status = 'active' OR role IN ('admin', 'owner', 'manager', 'staff')
          GROUP BY business_id, branch_id
        ) u ON d.business_id = u.business_id AND d.branch_id = u.branch_id
      ...
      await conn.query(`
        UPDATE devices d
        JOIN (
          SELECT business_id, MIN(id) as default_user_id FROM users GROUP BY business_id
    ...
    await ensureIndex(conn, "devices", "idx_devices_sku_user_status", "sku_id, user_id, status");
...
`;
    ...
    router3.put("/:id", async (req, res, next) => {
      ...
      try {
        ...
        await conn.execute(
          "INSERT INTO activity_logs (business_id, branch_id, product_id, user_id, user_name, activity_type, description) VALUES (?,?,?,?,?,?,?)",
  ...
  "src/routes/inventory.ts"() {
    ...
    router8.get("/devices/check-imei", async (req, res, next) => {
      ...
      try {
        ...
        const params = [cleanImei, cleanImei, req.user.business_id];
        let branchClause = "";
        if (branchId) {
          branchClause = " AND d.branch_id = ?";
          params.push(branchId);
        }
        const device = await queryOne(`
      SELECT d.id, d.imei, d.imei_serial, d.status, d.branch_id, d.user_id, d.condition, d.gb, d.color,
             COALESCE(p.name, 'Standard Mobile Device') as product_name, s.sku_code, b.name as branch_name, u.name as user_name
      FROM devices d
      LEFT JOIN product_skus s ON d.sku_id = s.id
      LEFT JOIN products p ON (d.product_id = p.id OR s.product_id = p.id)
      LEFT JOIN branches b ON d.branch_id = b.id
      LEFT JOIN users u ON d.user_id = u.id
      WHERE (d.imei = ? OR d.imei_serial = ?) AND d.business_id = ?${branchClause}
      LIMIT 1
    `, params);
        if (device) {
    ...
    router8.get("/devices/stats", async (req, res, next) => {
      ...
      FROM devices
      WHERE business_id = ?${branchClause}
    `, params);
...
"351500437920378","iPhone 13","Mobile Phones","Apple","128GB","Midnight","Grade A",220.00,330.00,"in_stock","Clean","Unlocked","2026-08-05 16:04:32"`;
    ...
    router8.get("/devices/export-csv", async (req, res, next) => {
      ...
      SELECT 
        ...
        COALESCE(p.name, 'Standard Mobile Device') as product_name,
...
`;
    ...
    router8.get("/devices/search", async (req, res, next) => {
      ...
      try {
        const branchId = branch_id ? Number(branch_id) : req.user.branch_id;
        let sql = `
      SELECT 
        d.*, 
        COALESCE(p.name, 'Standard Mobile Device') as product_name, 
    ...
    router8.get("/devices/:id", async (req, res, next) => {
      try {
        ...
        const device = await queryOne(`
      SELECT d.*, COALESCE(p.name, 'Standard Mobile Device') as product_name, s.sku_code, s.barcode,
      ...
      LEFT JOIN users u ON d.user_id=u.id
      WHERE d.id=? AND d.business_id=?${branchClause}
    `, params);
    ...
    router8.put("/devices/:id", async (req, res, next) => {
      ...
      try {
        ...
        const branchId = req.user.branch_id;
        const oldParams = [req.params.id, req.user.business_id];
        let oldBranchClause = "";
        if (branchId) {
          oldBranchClause = " AND d.branch_id=?";
          oldParams.push(branchId);
        }
        const [oldRows] = await conn.execute(
          `SELECT d.*, COALESCE(p.name, 'Standard Mobile Device') as product_name 
       FROM devices d 
       LEFT JOIN product_skus s ON d.sku_id=s.id 
       LEFT JOIN products p ON (d.product_id=p.id OR s.product_id=p.id) 
       WHERE d.id=? AND d.business_id=?${oldBranchClause}`,
          oldParams
        );
        const old = oldRows[0];
        if (!old) {
        ...
        let updateBranchClause = "";
        if (branchId) {
          updateBranchClause = " AND branch_id=?";
      ...
      WHERE id=? AND business_id=?${updateBranchClause}
    `, updateParams);
    ...
    router8.get("/devices/:id/activity", async (req, res, next) => {
      try {
        ...
        const dev = await queryOne(`SELECT id FROM devices WHERE id=? AND business_id=?${branchClause}`, checkParams);
    ...
    router8.post("/devices/:id/activity", async (req, res, next) => {
      ...
      try {
        ...
        const device = await queryOne(`SELECT id, branch_id, sku_id FROM devices WHERE id=? AND business_id=?${branchClause}`, checkParams);
    ...
    router8.delete("/devices/:id", async (req, res, next) => {
      ...
      try {
        ...
        let delBranchClause = "";
        if (branchId) {
          delBranchClause = " AND d.branch_id=?";
          delParams.push(branchId);
        }
        const [oldRows] = await conn.execute(
          `SELECT d.*, COALESCE(p.name, 'Standard Mobile Device') as product_name, p.id as product_id, s.id as sku_id 
       FROM devices d 
       LEFT JOIN product_skus s ON d.sku_id=s.id 
       LEFT JOIN products p ON (d.product_id=p.id OR s.product_id=p.id) 
       WHERE d.id=? AND d.business_id=?${delBranchClause}`,
          delParams
        );
        const dev = oldRows[0];
        if (!dev) {
        ...
        let finalDelClause = "";
        if (branchId) {
          finalDelClause = " AND branch_id=?";
          finalDelParams.push(branchId);
        }
    ...
    router8.get("/devices", async (req, res, next) => {
      ...
      SELECT d.id, d.business_id, d.branch_id, d.user_id, d.sku_id, d.imei, d.imei_serial, d.color, d.gb, d.ram,
             ...
             COALESCE(p.name, 'Standard Mobile Device') as product_name, s.sku_code, inv.invoice_number,
             b.name as branch_name, u.name as user_name
      FROM devices d
      LEFT JOIN product_skus s ON d.sku_id=s.id
      LEFT JOIN products p ON (d.product_id=p.id OR s.product_id=p.id)
        ...
        if (isCrossBusiness) {
         ...
         LIMIT 1`,
          ...
          if (existingSkuRows.length > 0) {
            destSkuId = existingSkuRows[0].sku_id;
            destProductId = existingSkuRows[0].product_id;
          }
          ...
          if (destProductId && !destSkuId) {
            ...
            if (prodSkus.length > 0) {
              destSkuId = prodSkus[0].id;
            } else {
              const [allProdSkus] = await conn.execute(
                "SELECT id, sku_code FROM product_skus WHERE product_id=?",
                [destProductId]
              );
              if (allProdSkus.length === 1 && transfer.device_id) {
                destSkuId = allProdSkus[0].id;
              } else {
                try {
