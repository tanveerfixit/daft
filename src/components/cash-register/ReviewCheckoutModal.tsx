...
export const ReviewCheckoutModal: React.FC<ReviewCheckoutModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 bg-black/65 flex items-center justify-center z-[110] p-3 sm:p-4 font-mono text-base animate-in fade-in duration-150">
      <div className="bg-white dark:bg-black border border-neutral-300 dark:border-neutral-800 w-full max-w-lg overflow-hidden flex flex-col rounded-lg shadow-2xl">
        
        {/* Header */}
        <div className="bg-neutral-200 dark:bg-neutral-900 px-5 py-3 border-b border-neutral-300 dark:border-neutral-800 flex justify-between items-center">
          ...
          <button 
            ...
            className="p-1 hover:bg-neutral-300 dark:hover:bg-neutral-800 text-neutral-600 dark:text-neutral-400 hover:text-black dark:hover:text-white transition-colors border-0 cursor-pointer disabled:opacity-30 rounded-sm"
          >
            <X size={22} />
          </button>
        </div>
        ...
        <div className="p-5 space-y-4 bg-white dark:bg-black">
          ...
          {/* Total Amount Box */}
          <div className="bg-neutral-100 dark:bg-neutral-900 border border-neutral-300 dark:border-neutral-800 p-5 flex flex-col items-center justify-center space-y-1 rounded-md">
            ...
            <div className="space-y-2">
              {payments.map((p, idx) => (
                <div key={idx} className="flex justify-between items-center py-1 text-base text-neutral-900 dark:text-neutral-100 font-sans">
                  <span className="font-bold bg-neutral-200 dark:bg-neutral-800 px-2 py-0.5 uppercase text-xs text-neutral-800 dark:text-neutral-200 rounded-sm">
                    {p.method}
                  </span>
          ...
          {changeDue > 0.005 && (
            <div className="bg-red-50 dark:bg-red-950/40 border-2 border-red-500 dark:border-red-600 p-4 flex justify-between items-center rounded-md shadow-xs">
          ...
          <div className="space-y-2 pt-2 border-t border-neutral-200 dark:border-neutral-800">
            ...
            <div className="grid grid-cols-2 gap-3 font-sans">
              <button
                ...
                disabled={isFinalizing}
                className={`py-3 flex items-center justify-center gap-2 border font-bold text-sm uppercase tracking-wider transition-all rounded cursor-pointer ${
        ...
        <div className="flex border-t border-neutral-300 dark:border-neutral-800 bg-neutral-100 dark:bg-neutral-950 p-4 justify-end gap-3 shrink-0">
          <button 
            onClick={onCancel}
            disabled={isFinalizing}
            className="flex-1 bg-white dark:bg-black border border-neutral-300 dark:border-neutral-800 hover:bg-neutral-200 dark:hover:bg-neutral-900 text-neutral-900 dark:text-neutral-100 font-bold py-3.5 px-4 rounded text-xs uppercase tracking-wider transition-colors cursor-pointer"
          >
            Cancel
          </button>
          <button 
            ...
            disabled={isFinalizing}
            className="flex-1 bg-amber-400 hover:bg-amber-500 text-slate-950 font-black py-3.5 px-5 rounded text-sm uppercase tracking-wider border border-amber-500 hover:border-amber-600 transition-all flex items-center justify-center gap-2 cursor-pointer disabled:opacity-50 shadow-sm"
          >
            {isFinalizing ? (
              <>
