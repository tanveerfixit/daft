...
export default function ProductFormModal({ onClose, onSave, initialData }: ProductFormModalProps) {
  ...
  const isDeveloper = currentUser?.role === 'developer' || currentUser?.email === 'support@techinbox.ie';
  ...
  const [formData, setFormData] = useState({
    ...
    tracking_type: initialData?.product_type === 'serialized' ? 'stock' : (initialData?.product_type === 'service' ? 'non-inventory' : (initialData?.product_type || 'stock')),
    has_serial: initialData?.product_type === 'serialized',
    is_taxable: true,
    require_note: false,
    min_stock_level: '0',
    allow_overselling: initialData?.allow_overselling !== 0,
    min_sales_price: '',
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    onSave({
      ...
      barcode: formData.barcode || formData.sku_code || '',
      allow_overselling: formData.allow_overselling
    });
  };

  return (
    <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-[100] p-4 font-mono text-base select-none">
      <div className="bg-white dark:bg-black border border-neutral-350 dark:border-neutral-800 w-full max-w-[760px] max-h-[90vh] overflow-y-auto flex flex-col rounded-none shadow-none">
        {/* Header */}
        <div className="bg-neutral-200 dark:bg-neutral-900 px-4 py-3 flex justify-between items-center border-b border-neutral-300 dark:border-neutral-800 rounded-none shrink-0">
          <h3 className="text-black dark:text-white font-bold text-lg uppercase tracking-wider">Edit Product</h3>
          <button onClick={onClose} className="text-neutral-500 hover:text-neutral-850 dark:hover:text-neutral-350 transition-colors">
            <X size={20} />
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} className="flex-1 p-5 space-y-5">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-1">
              ...
              <input
                ...
                className="w-full p-2 bg-[#ebf3fe] dark:bg-neutral-900 border border-neutral-300 dark:border-neutral-800 rounded-none text-base focus:outline-none focus:bg-white focus:border-amber-500 text-neutral-900 dark:text-neutral-100 font-normal disabled:opacity-75 disabled:cursor-not-allowed"
                value={formData.product_name}
                onChange={e => setFormData({ ...formData, product_name: e.target.value })}
                required
                disabled={initialData?.product_type === 'serialized' && !isDeveloper}
              />
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="space-y-1">
                <label className="text-[12px] font-bold text-neutral-500 uppercase tracking-wider block">Cost Price (€)</label>
                <input
                  type="number"
                  step="0.01"
                  placeholder="0.00"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-1">
                <label className="text-[12px] font-bold text-neutral-500 uppercase tracking-wider block">SKU Code</label>
                <input
                  type="text"
              ...
              <div className="space-y-1">
                <label className="text-[12px] font-bold text-neutral-500 uppercase tracking-wider block">Barcode</label>
                <input
                  type="text"
                  placeholder="e.g. Barcode987654"
                  className="w-full p-2 bg-[#ebf3fe] dark:bg-neutral-900 border border-neutral-300 dark:border-neutral-800 rounded-none text-base focus:outline-none focus:bg-white focus:border-amber-500 text-neutral-900 dark:text-neutral-100 font-mono"
          ...
          <div className="space-y-4 pt-1">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="space-y-1">
                ...
                <select
                  className="w-full p-2 bg-[#ebf3fe] dark:bg-neutral-900 border border-neutral-300 dark:border-neutral-800 rounded-none text-base focus:outline-none focus:bg-white text-neutral-900 dark:text-neutral-100 font-normal disabled:opacity-75 disabled:cursor-not-allowed"
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 pt-1">
              {formData.tracking_type === 'stock' && (
                <div className="flex items-center gap-2">
                  <input
                    type="checkbox"
                    id="edit_has_serial"
                    className="w-4 h-4 rounded-none accent-black dark:accent-white disabled:opacity-75 disabled:cursor-not-allowed"
                    checked={formData.has_serial}
                    onChange={e => setFormData({ ...formData, has_serial: e.target.checked })}
                    disabled={initialData?.product_type === 'serialized' && !isDeveloper}
                  />
                  <label htmlFor="edit_has_serial" className="text-sm font-bold text-neutral-700 dark:text-neutral-300 cursor-pointer">
                    Has Serial / IMEI ID
                  </label>
                </div>
              )}
              ...
              <div className="flex items-center gap-2">
                ...
                />
                <label htmlFor="edit_is_taxable" className="text-sm font-bold text-neutral-700 dark:text-neutral-300 cursor-pointer">
                  Taxable Product
                </label>
              </div>
              ...
              <div className="flex items-center gap-2">
                <input
                  type="checkbox"
                  id="edit_require_note"
                  className="w-4 h-4 rounded-none accent-black dark:accent-white"
                  checked={formData.require_note}
                  onChange={e => setFormData({ ...formData, require_note: e.target.checked })}
                />
                <label htmlFor="edit_require_note" className="text-sm font-bold text-neutral-700 dark:text-neutral-300 cursor-pointer">
              ...
              {formData.tracking_type === 'stock' && (
                <div className="flex items-center gap-2">
                  <input
                    type="checkbox"
                    id="edit_allow_overselling"
                    className="w-4 h-4 rounded-none accent-black dark:accent-white"
                    checked={formData.allow_overselling}
                    onChange={e => setFormData({ ...formData, allow_overselling: e.target.checked })}
                  />
                  <label htmlFor="edit_allow_overselling" className="text-sm font-bold text-neutral-700 dark:text-neutral-300 cursor-pointer">
                    Allow Overselling
                  </label>
                </div>
              )}
          ...
          <div className="flex justify-end items-center gap-3 pt-3 border-t border-neutral-200 dark:border-neutral-800 bg-neutral-100 dark:bg-neutral-950 -mx-5 -mb-5 p-4 shrink-0">
            <button
              type="button"
              onClick={onClose}
